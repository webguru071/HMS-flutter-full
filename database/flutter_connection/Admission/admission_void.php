...
$stmt = $conn->prepare(
	...
	WHERE hms_admissions.patient_id = patients.id AND hms_admissions.bed_id = hms_beds.id AND hms_admissions.referral_id = referrals.id AND hms_admissions.status = 'Void'
	ORDER BY hms_admissions.id DESC" );

$stmt->execute();

//binding results to the query 
$stmt->bind_result($id,$admit_date, $patient_name, $invoice,  $paid_amount, $due, $name);
